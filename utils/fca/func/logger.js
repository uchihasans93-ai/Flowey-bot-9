...
module.exports = (text, type) => {
  ...
  if (s === "warn") {
    process.stderr.write(co(`\r[ FCA-UZAIR-WARN ] > ${text}`) + "\n");
    return;
  }
  if (s === "error") {
    process.stderr.write(chalk.bold.hex("#ff0000")(`\r[ FCA-UZAIR-ERROR ]`) + ` > ${text}\n`);
    return;
  }
  if (s === "info") {
    process.stderr.write(chalk.bold(co(`\r[ FCA-UZAIR ] > ${text}`)) + "\n");
    return;
  }
