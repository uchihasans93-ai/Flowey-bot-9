# Fca-hakim-tracks-mtx - Complete API Documentation

## Introduction

**fca-hakim-tracks-mtx** is an uzair Node.js library for interacting with Facebook Messenger by emulating browser behavior. This library allows you to create chat bots and automate tasks on Facebook Messenger.

## Installation

```bash
npm install fca-hakim-tracks-mtx@latest
```
...
```javascript
const login = require("fca-hakim-tracks-mtx");
...
```javascript
api.sendMessage("Hello!", "61552682190483", (err, messageInfo) => {
    if (err) {
...
// Event type: "message"
{
    ...
    participantIDs: ["61552682190483"]  // Array of participant IDs
}
...
api.useMiddleware("blockUsers", (event, next) => {
    if (event.type === "message") {
        const blockedUsers = ["61552682190483", "61552682190484"];
...
login({ appState: [] }, (err, api) => {
    ...
    // 2. Block spam users
    const spamUsers = ["61552682190483"];
...
```javascript
// Get info for 1 user
api.getUserInfo("61552682190483", (err, userInfo) => {
    if (err) {
        console.error(err);
        return;
    }
    ...
    //     vanity: "john.doe",
    //     thumbSrc: "avatar_url",
    //     profileUrl: "https://facebook.com/mtxuzair",
...
api.getUserInfo(["61552682190483", "61552682190482"], (err, userInfo) => {
    if (err) return console.error(err);
...
```javascript
// Add 1 person
api.addUserToGroup("61552682190483", "6155268219048", (err) => {
    if (err) {
...
// Add multiple people
api.addUserToGroup(["61552682190483", "615526821904832"], "61552682190", (err) => {
    if (err) return console.error(err);
...
```javascript
api.removeUserFromGroup("61552682190483", "1234567890", (err) => {
    if (err) {
...
```javascript
api.changeNickname("Admin Bot", "1234567890", "61552682190483", (err) => {
    if (err) {
...
api.changeNickname("", "1234567890", "61552682190483", (err) => {
    if (err) return console.error(err);
...
api.listenMqtt((err, event) => {
    ...
    if (event.type === "message" && event.attachments.length > 0) {
        ...
        api.forwardAttachment(attachmentID, "61552682190483", (err) => {
            if (err) {
...
```javascript
// Block user
api.changeBlockedStatus("61552682190483", true, (err) => {
    if (err) {
        console.error("Error:", err);
        return;
    }
...
// Unblock
api.changeBlockedStatus("61552682190483", false, (err) => {
    if (err) return console.error(err);
...
```javascript
const members = ["61552682190483", "615526821904832", "6155268219048303"];
...
// Admin list (User IDs)
const ADMINS = ["61552682190483", "615526821904832"];
...
- **GitHub Repository**: https://github.com/MrUzairXxX-MTX-PROJECT/fca-hakim-tracks-mtx
- **NPM Package**: https://www.npmjs.com/~hakim-tracks
...
This is a comprehensive documentation for **fca-hakim-tracks-mtx** API methods. This library is very powerful but should be used carefully to avoid violating Facebook's policies and getting your account banned.
